...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_rules'])) {
    ...
    if (!validateCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        // [แก้ไข] กฏการลา (เปลี่ยนจาก monthly เป็น yearly แยกประเภท 3 ตัว + 2 ตัวเดิม = 5 ตัว)
        $max_concurrent_leave = (int)($_POST['max_concurrent_leave'] ?? 3);
        $work_days_before_leave = (int)($_POST['work_days_before_leave'] ?? 5);
        $yearly_sick_leave = (int)($_POST['yearly_sick_leave'] ?? 30);
        $yearly_personal_leave = (int)($_POST['yearly_personal_leave'] ?? 6);
        $yearly_vacation_leave = (int)($_POST['yearly_vacation_leave'] ?? 10);
        
        // เวลาทำงาน (2 ตัวแปร)
        $work_start_time = $_POST['work_start_time'] ?? '08:00';
        $work_end_time = $_POST['work_end_time'] ?? '17:00';

        // รวมจำนวน Integer ทั้งหมด: 8(ปกติ) + 8(หยุด) + 5(ลาใหม่) = 21 ตัว
        ...
        try {
            if ($current_rules) {
                // === UPDATE ===
                // [แก้ไข] SQL Update เปลี่ยน monthly_leave_days เป็น yearly_xxx 3 ตัว
                $update_sql = "UPDATE schedule_rules SET 
                              ...
                              max_concurrent_leave = ?, work_days_before_leave = ?, 
                              yearly_sick_leave = ?, yearly_personal_leave = ?, yearly_vacation_leave = ?,
                ...
                // สร้าง Type String: ss + 21i + ss + i = 26 chars
                $types = "ss" . str_repeat("i", 21) . "ssi";
                ...
                mysqli_stmt_bind_param($stmt, $types, 
                    ...
                    $max_concurrent_leave, $work_days_before_leave, // 2i
                    $yearly_sick_leave, $yearly_personal_leave, $yearly_vacation_leave, // 3i (New)
                ...
                // สร้าง Type String: iss + 21i + ss + i = 27 chars
                $types = "iss" . str_repeat("i", 21) . "ssi";
                ...
                mysqli_stmt_bind_param($stmt, $types, 
                    ...
                    $holiday_morning_afternoon_count, $holiday_morning_night_count, $holiday_afternoon_night_count,
                    $max_concurrent_leave, $work_days_before_leave, 
                    $yearly_sick_leave, $yearly_personal_leave, $yearly_vacation_leave, // 21i
                    $work_start_time, $work_end_time, // ss
                    $_SESSION['user_id'] // i
                );
                mysqli_stmt_execute($stmt);
            ...
            // Save Employee Levels (No changes here)
            $levels = [1, 2, 3];
            ...
            // Save Holiday Rules (No changes here)
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post">
                            ...
                            <h4 class="section-title">กฏการลา (โควตาต่อปี)</h4>
                            <div class="rules-grid">
                                <div class="rule-item">
                                    <label>ลาป่วย (วัน/ปี)</label>
                                    <input type="number" name="yearly_sick_leave" class="form-control" min="0" 
                                           value="<?php echo $current_rules['yearly_sick_leave'] ?? 30; ?>">
                                </div>
                                <div class="rule-item">
                                    <label>ลากิจ (วัน/ปี)</label>
                                    <input type="number" name="yearly_personal_leave" class="form-control" min="0" 
                                           value="<?php echo $current_rules['yearly_personal_leave'] ?? 6; ?>">
                                </div>
                                <div class="rule-item">
                                    <label>ลาพักร้อน (วัน/ปี)</label>
                                    <input type="number" name="yearly_vacation_leave" class="form-control" min="0" 
                                           value="<?php echo $current_rules['yearly_vacation_leave'] ?? 10; ?>">
                                </div>
                                <div class="rule-item">
